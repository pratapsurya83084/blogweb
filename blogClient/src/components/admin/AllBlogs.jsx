...
const AllBlogs = () => {
  ...
  const api=await axios.post("http://localhost/blogweb/backend/admin/deleteBlogbyidwise.php",
  {
    id:id
            ...
            <table className="min- bg-white shadow rounded-lg table-auto lg:w-full">
              <thead>
                <tr className="bg-gray-200">
                  ...
                  <th className="p-2 text-left text-xs md:text-">
                    Blog Subtitle
                  </th>
              ...
              <tbody>
                {blogData.map((blog, index) => (
                  <tr key={blog.id} className="border-t">
                    <td className="p-2 text-xs md:text-xs lg:text-">
                      {index + 1}
                    </td>
                    <td className="p- text-xs md:text-xs lg:text-md">
                      {blog.blog_title}
                    </td>
                    <td className="p-2 text-xs md:text-xs lg:text-md">
                      {blog.blog_subtitle}
                    </td>
                    <td className="p-2 text-xs md:text-xs lg:text-md">
                      {blog.blog_category}
                    ...
                    <td className="p-2  ">
                      <img
                        className="h-6 w-6 lg:h-14 lg:w-14"
                        src={blog.blog_img}
                        alt=""
                      />
                    </td>
